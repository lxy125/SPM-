...
public class BookService {
    ...
    public BookService(RestTemplate restTemplate,
                       @Value("${isbn.api.key}") String apiKey,
                       @Value("${isbn.api.url}") String apiUrl) {
        this.restTemplate = restTemplate;
        this.apiKey = apiKey;
        this.apiUrl = apiUrl;
    }

    public BookInfo fetchBookDetailsFromAPIByIsbn(String isbn) {
        String url = String.format("%s?key=%s&isbn=%s", apiUrl, apiKey, isbn);
        logger.debug("Fetching book details from API: {}", url);

        BookInfo.BookData response = restTemplate.getForObject(url, BookInfo.BookData.class);
        logger.debug("Received book details: {}", response != null ? response.getData() : null);
    ...
    public boolean canDeleteBook(int bookId) {
        return bookMapper.countUnreturnedBooks(bookId) == 0;
    }
